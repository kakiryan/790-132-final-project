...
Check string_dec. 
Check string_dec "s" "s".
Compute string_dec "s" "s".
Compute string_dec "s" "d".
...
Lemma t_update_eq : forall (A : Type) (m : total_map A) x v,
  (x !-> v ; m) x = v.
Proof.
  intros A m x v. unfold t_update. rewrite <- eqb_string_refl. reflexivity. Qed.
(** [] *)
...
Theorem t_update_neq : forall (A : Type) (m : total_map A) x1 x2 v,
  ...
  (x1 !-> v ; m) x2 = m x2.
Proof.
  intros A m x1 x3 v H. unfold t_update. apply eqb_string_false_iff in H. rewrite H.
reflexivity. Qed.
(** [] *)
...
Lemma t_update_shadow : forall (A : Type) (m : total_map A) x v1 v2,
  (x !-> v2 ; x !-> v1 ; m) = (x !-> v2 ; m).
Proof.
  intros A m x v1 v2. apply functional_extensionality. unfold t_update. intros x0.
  destruct (eqb_string x x0). reflexivity. reflexivity. Qed.
(** [] *)
...
Proof.
  intros x y. unfold eqb_string. destruct (string_dec x y) eqn:E.
...
Lemma eqb_stringP' : forall x y : string,
  reflect (x = y) (eqb_string x y).
Proof.
  intros x y. apply iff_reflect. rewrite eqb_string_true_iff. reflexivity. Qed.
...
Theorem t_update_same : forall (A : Type) (m : total_map A) x,
  (x !-> m x ; m) = m.
Proof.
 intros A m x. apply functional_extensionality. intros x0. unfold t_update. 
