...
Fixpoint oddmembers (l:natlist) : natlist
  := match l with
  | nil => nil
  | h :: t => if even (hd 0 l) then oddmembers t else (h :: oddmembers t)
end. 
...
Fixpoint alternate (l1 l2 : natlist) : natlist
 := match l1, l2 with
  ...
  | nil, yh :: yt => l2
  | xh :: xt, nil => l1
  | xh :: xt, yh :: yt =>  xh :: yh :: (alternate xt yt)
end.
...
Fixpoint count (v : nat) (s : bag) : nat
  := match v, s with 
  | x, nil => O
  | x, h :: t => if x =? h then S (count v t) else count v t
end.
...
:= if (count v s) =? O then false else true.
...
Theorem fool: forall n: nat, forall l: natlist,
repeat n 0 = l -> length l = 0.
intros n l H. rewrite <- H. reflexivity. Qed.

Theorem lemma: forall m n: nat, length (repeat n m) = m.
...
intros n m l H. destruct l as [|n' l'] eqn:E.
- induction m as [|m' IHm'].
  -- simpl. reflexivity.
  -- rewrite <- H. rewrite lemma. reflexivity.
- rewrite <- H. rewrite lemma. reflexivity.
Qed.

Theorem foo2': forall n m : nat, forall l: natlist,
repeat n m = l -> length l = m.
intros n m l H. induction l as [|n' l'] eqn:E.
- destruct m as [|m'].
  -- simpl. reflexivity.
  -- rewrite <- H. rewrite lemma. reflexivity.
- rewrite <- H. rewrite lemma. reflexivity.
...
Theorem foo2'': forall n m : nat, forall l: natlist,
repeat n m = l -> length l = m.
Proof. induction m as [|m' IH].
- simpl. intros. rewrite <- H. reflexivity.
 - simpl. intros. rewrite <- H. simpl. rewrite IH. reflexivity. reflexivity. Qed.

(** Theorem foo3: forall n: nat, forall l1 l2: natlist,
l2 ++ [n] = l2 -> (1 <=? (length l2)) = true.
Proof.
intros n l1 l2 H. rewrite <- H. induction l2 as [| m l2' IHl2'].
- simpl. reflexivity.
- simpl. reflexivity.
Qed. *)
...
Theorem bag_theorem : forall (b: bag) ( n: nat), count n (add n b) =  S (count n b).
Proof. intros n b. simpl. rewrite <- eqb_refl. reflexivity. Qed.

(*
Theorem bag_theorem : forall (b: bag) ( n: nat), count n (add n b) =  S (count n b).
Proof.
  intros n b. simpl. rewrite <- eqb_refl. reflexivity.
Qed.
*)
...
(* FILL IN HERE
Theorem bag_count_sum: forall (s1 s2: bag) (n: nat), count n (sum s1 s2) = count n s1 + count n s2.
Proof.
  intros s1 s2 n. induction s1 as [|n' s1' IHs'].
  - simpl. reflexivity.
  - simpl. destruct (n =? n').
    -- simpl. rewrite IHs'. reflexivity.
...
(* The type baz has no elements that satisfy its specification. It's basically a recursive
type with no base case. To construct any element of type baz, another element of type baz 
is required. SO the answer here is zero. *)
