...
Proof.
  ...
  injection H1 as H3 H4. apply trans_eq with (m:= z).
  - apply H3.
  - assert (H5: y :: l = z :: l).
    * rewrite H4. rewrite H2. reflexivity.
    * injection H5. symmetry. apply H. Qed.
(** [] *)
...
Example discriminate_ex3 :
  forall (X : Type) (x y z : X) (l : list X),
    ...
    x = z.
Proof.
  intros X x y z l H. discriminate H. Qed.
(** [] *)
...
Theorem ex: forall (x y: nat),
  1 + x =  1 + y -> y = x.
Proof.
  intros x y H.
  injection H. intros h1. symmetry. apply h1. Qed.
...
(* We want to prove that for all natural numbers n, m; if n =? m is true, then n = m.

First we will perform induction on n.

Base case: n = 0. The subproof here is that for all m, if 0 =? m is true, then 0 = m.
We can show this by case analysis of m.

1. m = 0. Showing 0 =? 0 -> 0 = 0 is straightforward.
2. m = S m'. Now we want to show 0 =? Sm' -> 0 = S m'. This follows directly from the fact 
that 0 =? Sm' can never be true.

At this point, we have shown the base case is true for all m.

Next, the inductive case, so our inductive hypothesis that for all m, if  n' =? m is 
true, then n' = m. We want to prove this is true for Sn'. We proof this half with a similar case analysis of m.

1. m = 0. Showing Sn' =? 0 -> Sn' = 0 follows from the fact that Sn'=? 0 can never be true.
2. m = Sm'. Now we want to show Sn' =? Sm' -> Sn' = Sm'. From the injective property of
the constructors for nats, we can simply Sn' = Sm' to be n' = m'.
From here we can directly apply the induction hypothesis and we are done.*) 
...
Proof.
  ...
  - intros m eq. destruct m as [| m'] eqn:E.
    * discriminate eq.
    * apply f_equal. apply IHn'. simpl in eq. rewrite <- plus_n_Sm in eq.
  rewrite <- plus_n_Sm in eq. injection eq as H2. apply H2. Qed.
(** [] *) 
...
Proof.
  intros n X l H.
  generalize dependent n. induction l as [| h t IHl'].
  - simpl. reflexivity.
  - intros n H. destruct n as [|n'] eqn:E.
    * simpl in H. discriminate H. 
    * rewrite <- H. simpl. apply IHl'. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - intros l1 l2 H. simpl in H. injection H as H2. rewrite <- H. rewrite <- H2. simpl. reflexivity.
  - destruct h as [x y] eqn:E1. simpl. destruct (split t)  as (lx, ly) eqn:E2. 
    intros l1 l2 H. injection H as E3 E4.
    rewrite <- E3. rewrite <- E4. simpl. rewrite IHl'.
    * reflexivity.
    * reflexivity.
Qed.

(** The [eqn:] part of the [destruct] tactic is optional; though we've
    chosen to include it most of the time, for the sake of
    documentation, but it can often be omitted without harm.
...
Proof.
  intros n. induction n as [| n' IHn'].
  - intros m. destruct m as [|m'] eqn:M.
    * simpl. reflexivity.
    * simpl. reflexivity.
  - intros m. destruct m as [|m'] eqn:M.
    *  simpl. reflexivity.
...
Theorem eqb_trans : forall n m p,
  ...
  n =? p = true.
Proof.
  intros n m p H1 H2. apply trans_eq with (m:= n =? m). apply f_equal. apply eqb_true in H2.
  symmetry. apply H2. apply H1.
 Qed.
(** [] *)
...
Proof.
 intros X Y l1. induction l1 as [| h t IHl'].
  - intros l2 H. destruct l2 as [|e l2'] eqn:E.
    * simpl. reflexivity.
    * rewrite <- E. simpl. discriminate H.
  - intros l2 H. destruct l2 as [|e l2'] eqn:E.
    * discriminate H.
    * injection H as H2. apply IHl' in H2. simpl. rewrite H2. reflexivity. Qed.
...
Fixpoint forallb {X : Type} (test : X -> bool) (l : list X) : bool
  := match l with
  | [] => true
  | x :: t => andb (test x) (forallb test t)
end.
