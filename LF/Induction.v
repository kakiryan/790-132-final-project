...
With the destruct tactic, we are introducing all of the possible cases and proving 
each one directly. With induction, are just looking at a base case and then the 
inductive case. Proving the induction hypothesis allows us to prove all cases at once.
*)
...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    Theorem: Addition is commutative. In other words, for all naturals [n], 
  [m], the following is true: n + m = m + n.

    Proof: By Induction on [n]

  - First, let's assume [n = 0]. We want to show 0 + m = m + 0. This simplifies
  to m = m + 0, by the definition of plus. Next we can rewrite using the add_0_r 
  theorem previously proven to get m = m, which is true.

  - Next, suppose for some n', n' + m = m + n'.
    We must now show that

        (S n') + m = m + (S n').

  First, we can simplify to S (n' + m) = m + (Sn'), and then rewrite using the induction 
  hypothesis to get:
      
      S (m + n') = m + Sn'.

  Then, using the theorem plus_n_Sm, we can rewrite the left side and get:

    m + Sn'= m + Sn'. And we are done!
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    - First, assume [n = 0]. We want to show true = 0 =? 0. This follows from
  our definition of eqb on natural numbers.

    - Next, assume true = n' =? n' for some n'. We want to show: true = S n' =? S n'.
    This also follows from our definition of eqb and the induction hypothesis.
...
Theorem mul_helper: forall n k: nat,
n * (S k) = n + n * k.
Proof.
  intros n k. induction n as [|n' IHn'].
  - reflexivity.
  - simpl. rewrite -> IHn'. rewrite -> add_shuffle3. reflexivity. Qed.
...
Proof.
  ...
  - simpl. rewrite -> mul_helper. rewrite -> IHn'. reflexivity. Qed.
(** [] *)
...
Theorem add_shuffle3' : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  intros n m p. rewrite add_comm. replace (n+p) with (p+n).
...
(*The problem with this is that the same binary numbers can be represnted 
with multiple different compositions of constructors. So 0 could be just Z or B0 Z.*)
...
Definition normalize_helper(b: bin): bin :=
 (* Account for the issue in part b. here *)
 match b with
    | Z => Z
    | B0 b' => B0 (B0 b')
    | B1 b'=> B0 (B1 b')
...
Fixpoint normalize (b: bin): bin :=
  match b with 
  | Z => Z
  | B0 b' => normalize_helper (normalize b')
  | B1 b' =>  incr (normalize_helper (normalize b'))
end.

Compute normalize (Z).
Compute normalize (B1 Z).
Compute normalize (B1 (B1 Z)).
Compute normalize (B1 (B1 Z)).
Compute normalize (B0 (B0 Z)).
Compute normalize (B0 Z).

Theorem lemma1: forall (b: bin), incr (incr (normalize_helper b)) = normalize_helper(incr b).
Proof.
...
Theorem lemma2 : forall (n: nat), nat_to_bin (n + n) = normalize_helper (nat_to_bin n).
Proof.
intros n. induction n as [|n' IHn'].
  -simpl. reflexivity.
  -simpl. rewrite <- plus_n_Sm. simpl. rewrite -> IHn'. rewrite -> lemma1. reflexivity. 
Qed.
  
Theorem lemma3: forall (n: nat), nat_to_bin (n * 2) = normalize_helper(nat_to_bin n).
Proof.
 intros n. induction n as [|n' IHn']. 
 - simpl. reflexivity.
 - simpl. rewrite -> IHn'. rewrite -> lemma1. reflexivity. Qed.

Theorem bin_nat_bin : forall b, nat_to_bin (bin_to_nat b) = (normalize b).
Proof.
  intros b. induction b as [|b' IHb' | b'' IHb''].
  - simpl. reflexivity.
  - simpl. rewrite -> lemma3. rewrite -> IHb'. reflexivity.
  - simpl. rewrite -> lemma3. rewrite -> IHb''. reflexivity. Qed.
