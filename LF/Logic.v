...
Check 3 + 2 =? 5.
Check forall n: nat, (((3_2) =? n) = true) -> n= 5.
(* Check forall n: nat, S (pred n). *)
...
(** **** Exercise: 1 star, standard (mul_eq_0) *)
Lemma mul_eq_0 :
...
Theorem xx: forall X, forall a b: X, (a=b) /\ (~ a=b) -> False.
Proof.
intros X a b [Hl Hr]. unfold not in Hr. apply Hr. apply Hl. Qed.

Theorem xy: forall P Q : Prop, P \/ Q -> ~~(P \/ Q).
  intros P Q H. unfold not. intros H2. apply H2. apply H. Qed.

Theorem xz: forall A B: Prop, A -> (A \/ ~~B).
Proof. intros A B H. unfold not. left. apply H. Qed. 

Theorem xw: forall A B: Prop, A -> (A \/ ~~B).
Proof. intros A B H. left. apply H. Qed.

Theorem z: forall P Q: Prop, P \/ Q -> ~~P \/ ~~Q.
Proof. intros P Q H. destruct H.
- left. unfold not. intros H2. apply H2. apply H.
- right. unfold not. intros H2. apply H2. apply H. Qed.

Theorem y: forall A : Prop, 1=0 -> (A \/ ~A).
  intros A. discriminate. Qed.
...
(* First, we assume P is true. If we additionally assume that
 ~P is true, we get a contradiction (P /\ ~P). Since this 
 assumption of ~P leads to a contradiction (a statement that 
 is necessarily false) that means ~~P must be true. 
 In conclusion if P is true, it follows that ~~P is also true.   *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H1 H2 H3. unfold not in H2. apply H2. apply H1. apply H3. Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P). 
Proof.
  intros P H. unfold not in H. apply H. apply H. Qed. 
(** [] *)
...
(* We are trying to show that P /\ ~P is False.
   So if we assume P is true and also that ~P is true we get a contradiction.
   So this means (P /\ ~P) is False so ~(P /\ ~P) must be true. *)
...
Theorem iff_refl : forall P : Prop,
  P <-> P.
Proof.
  intros P. split. intros H1. apply H1. intros H2. apply H2. Qed.
...
Proof.
  ...
  split. 
  - intros H1. apply HQ'. apply HP. apply H1.
  - intros H2. apply HQ. apply HR. apply H2. Qed.
(** [] *)
...
Lemma apply_iff_example :
  forall n m : nat, n * m = 0 -> n = 0 \/ m = 0.
Proof.
  intros n m H. rewrite <- mult_0. apply H.
Qed.
...
Proof.
  ...
  - intros [[x' HL] | [x' HR]].
    * exists x'. left. apply HL.
    * exists x'. right. apply HR.
Qed. 
(** [] *)
...
Proof.
  ...
  - (* l = x' :: l' *)
    simpl. intros [Hl | Hr].
    + rewrite Hl. left. reflexivity.
    + right. apply IHl'. apply Hr.
Qed.
  ...
  - intros H. induction l as [|h t IHl'].
    ...
    * simpl. destruct H as [HL | HR].
      --  exists h. split.
          **  apply HL.
          ** left. reflexivity.
      -- apply IHl' in HR. destruct HR as [x' HR'].
       exists x'. split.
           **  apply proj1 in HR'. apply HR'.
           ** right. apply proj2 in HR'. apply HR'.
  - intros H. induction l as [|h t IHl'].
    * simpl in H. destruct H as [x' H']. apply proj2 in H'. destruct H'.
    * simpl. simpl in H.  destruct H as [x' H']. destruct H' as [HL  HR].
    destruct HR as [HR1 | HR2].
      --  left.  rewrite HR1. apply HL.
...
Proof.
  ...
  - split. 
      * intros H. simpl. simpl in H. right. apply H.
      * intros H. simpl. simpl in H. destruct H as [HL | HR].
        -- destruct HL.
        --  apply HR.
  - split. intros H.
      *  simpl. simpl in H. destruct H. 
        -- left. left. apply H.
        -- apply IH in H. apply or_assoc. right.  apply H.
    * intros H. simpl. simpl in H. apply or_assoc in H.
...
Proof.
  ...
  - intros H1 x H2.  induction l as [| h t IHl].
    * simpl in H2. destruct H2.
   * destruct H1 as [H1L H1R]. destruct H2 as [H2L | H2R].
    -- rewrite H2L in H1L. apply H1L.
    --  apply IHl. 
        **  apply H1R.
        ** apply H2R.
Qed.
...
Proof.
  intros Podd Peven n H1 H2.
  unfold combine_odd_even in H1. rewrite H2 in H1. apply H1. Qed.
...
Lemma quiz6: forall (a b: nat), a=b -> b=42 -> True.
Proof.
  intros a b H1 H2. Check (trans_eq nat a b 42 H1 H2).
Abort.

Lemma quiz7: forall (a b: nat), a=b -> b=42 -> True.
Proof.
  intros a b H1 H2. Check (trans_eq nat b 42 a H2).
Abort.

Lemma quiz8: forall (a b: nat), a=b -> b=42 -> True.
Proof.
  intros a b H1 H2. Check (trans_eq _ 42 a b).
Abort.

(* Lemma quiz9: forall (a b: nat), a=b -> b=42 -> True.
Proof.
  intros a b H1 H2. Check (trans_eq _ _ _ _ H2 H1). *)
...
Lemma tr_rev_correct_helper: 
  forall X (l1 l2: list X), rev_append l1 l2 = rev l1 ++ l2.
Proof.  
  intros X l1. induction l1 as [| h t IHl'].
  -  simpl.  reflexivity.
  - intros l2. simpl.  rewrite IHl'. rewrite <- app_assoc. reflexivity. Qed.

Theorem tr_rev_correct : forall X, @tr_rev X = @rev X.
Proof.
 intros X. apply functional_extensionality.
 intros x. unfold tr_rev. simpl. induction x as [|x t IHl'].
  -  simpl.  reflexivity.
  -  simpl. rewrite <- IHl'. rewrite IHl'. apply tr_rev_correct_helper. Qed.
(** [] *)
...
Proof.
  ...
  - rewrite even_S. destruct (even n').
    ...
    * destruct IHn. simpl. exists (S x). rewrite H. simpl. reflexivity. Qed.
...
Proof.
  ...
  - intros H. destruct b1 eqn:E.
    ...
    * split.
      --  discriminate.
      -- discriminate H.
  - intros H. destruct H as [HL HR]. destruct b1 eqn:E.
    * apply HR.
    * discriminate HL.
Qed.
...
Proof.
  intros x y. split.
  -  unfold not. intros H1 H2. rewrite H2 in H1. rewrite <- eqb_refl in H1. discriminate H1.
  - unfold not. intros H. destruct (eqb x y) eqn:E.
...
Proof.
   intros A eqb H.  induction l1 as [|h t IHl']. split.
  - destruct l2 as [|l'] eqn:L. 
     * simpl. intros H2. reflexivity.
  ...
  - destruct l2 as [|l'] eqn:L.
    *  simpl. split.
      ...
      -- intros H2. discriminate.
    * split.
    --  intros H2. apply andb_true_iff in H2. destruct H2 as [H3 H4]. 
  apply H in H3. apply IHl' in H4. rewrite H3. rewrite H4. reflexivity.
   -- intros H2. injection H2 as H3 H4. apply H in H3. apply IHl' in H4.
    simpl. rewrite H3. rewrite H4. reflexivity.
Qed.
...
Proof.
  ...
  - intros H. induction l as [| h t IHl].
    *  simpl. reflexivity.
    * simpl. apply andb_true_iff in H. destruct H as [H1 H2]. apply IHl in H2.
      split.
        -- apply H1.
        -- apply H2.
  - intros H. induction l as [| h t IHl].
...
(* I'm not sure. None are standing out to me? *)
...
Proof.
  ...
  apply H. right. intros H2. apply H. left. apply H2. Qed.
(** [] *)
...
Proof.
   intros excluded_middle X P. unfold not. intros H1 x. destruct (excluded_middle (P x)) as [E1 | E2].
    -  apply E1.
    -  exfalso. apply H1. exists x. apply E2.
Qed.
(** [] *)
...
Proof. 
  ...
  intros HP P HD. apply (HP P False). intros H.  destruct H. destruct HD. unfold not.
Admitted.
