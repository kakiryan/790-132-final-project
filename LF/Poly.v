...
(** **** Exercise: 1 star, standard, optional (combine_checks)
    ...
    - What is the type of [combine] (i.e., what does [Check
      @combine] print?) It produces a lisst of pairs of elements of
     type X and Y.
    - What does
...
Definition filter_even_gt7 (l : list nat) : list nat
  := filter (fun n => andb  (negb (leb n  7)) ( even n)  ) l.
...
Theorem lemma_map_f_assoc: forall (X Y: Type) (f: X -> Y) (l1 l2: list X),
  map f (l1 ++ l2) = map f l1 ++ map f l2.
Proof. intros X Y f l1 l2. induction l1 as [| h t IHl].
  - simpl. reflexivity.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'.  rewrite lemma_map_f_assoc. reflexivity. Qed.
(** [] *)
...
(* One situation I could imagine would be if you had a list of strings, for example,
and you wanted to get the sum of all the characters. So this result would be an integer.
So X would be a str, f would take str -> int and add it to the running total.

Similarly, with types we've already seen in Coq, maybe you would want to get the total 
'true' bool values in a list. So X would be bool and Y would be nat.  *)
...
*)
(* Forall X l n, we assume length l = n and we want to prove that this implies
@nth_error applied to X l n will result in None. 

I will prove this by induction on l.

First, I will prove this with the base case that l = []
So if l is empty, the length = 0 => n = 0. So @nth_error X [] 0 = None is True because
nth_error always returns None with an empty list.

Next is the inductive case. We assume the inductive hypothesis to be true.
IH: If length l = n, then nth_error X 1 n = None

So now we want to prove this for some length l', n' that nth_error X l' n' = None.
We know that n' = length of l' so we can plug in and get: nth_error X l' length l' = None

(nth_error X some_list (length some_list)) will always return None. This can be similarly proved by 
induction but the basic idea is that since we start at 0, you are going out of bounds if you try to 
access the element at the position equal to the length of the list.

From here, it immediately follows that forall X l n, length l = n -> @nth_error X l n = None

I attempted to write some coq for this below, but it may not be the most efficient way. *)

Theorem helper_lemma: forall X l, @nth_error X l (length l) = None.
Proof. intros X l. induction l as [|h l IHl'].
 - simpl. reflexivity.
 - simpl. rewrite IHl'. reflexivity. Qed.

Theorem length_nth_error: forall X l n, length l = n -> @nth_error X l n = None.
Proof. intros X l n H.  induction l as [| h t IHl'].
  - simpl. reflexivity. 
  - rewrite <- H. rewrite helper_lemma. reflexivity.
Qed.
...
Definition exp' (n m : cnat) : cnat 
:= fun (X: Type) (f: X -> X) (x: X) => (m (X -> X) (fun (f : X -> X) => n X f) f) x.
